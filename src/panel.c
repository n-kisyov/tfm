...
void panel_go_parent(Panel *p, const FsProvider *fs) {
    ...
    PanelTab *tab = &p->tabs[p->active_tab];
    /* for SSH paths, go back to drive list */
    if (wcsncmp(tab->path, L"\\\\ssh\\", 6) == 0) {
        panel_go_drives(p, fs);
        return;
    }
    if (is_root_path(tab->path)) {
        panel_go_drives(p, fs);
        return;
    }
...
void panel_go_drives(Panel *p, const FsProvider *fs) {
    ...
    if (p->saved_entry_count > 0 && p->saved_entry_count <= 4096 && p->entries)
        memcpy(p->saved_entries, p->entries, p->saved_entry_count * sizeof(FileEntry));

    free(p->entries);
    p->entries = (FileEntry *)calloc(64, sizeof(FileEntry));
    p->entry_count = 0;
    p->in_drive_list = 1;
    ...
    for (int i = 0; i < 26; i++) {
        if (drives & (1 << i)) {
            ...
            if (p->drive_paths[i][0])
                swprintf_s(p->entries[p->entry_count].name, 256, L"%c:\\    (%s)", dl, p->drive_paths[i]);
            else
    ...
    for (int i = 0; i < g_app.ssh_config.count && p->entry_count < 60; i++) {
...
void panel_enter_on_drive(Panel *p, const FsProvider *fs) {
    ...
    /* save entry name before freeing */
    wchar_t entry_name[256];
    wcscpy_s(entry_name, 256, e->name);
    ...
    if (wcsncmp(entry_name, L"[ssh]", 5) == 0) {
        const wchar_t *name_start = entry_name + 6;
        const wchar_t *sp = wcschr(name_start, L' ');
        wchar_t conn_name[64] = {0};
        if (sp) wcsncpy_s(conn_name, 64, name_start, (size_t)(sp - name_start));
        ...
        panel_refresh(p, p->fs);
        /* if SSH listing failed, return to drive list */
        if (p->entry_count == 0) {
            p->fs = &fs_local;
            wcscpy_s(tab->path, 520, L"Drives");
            wcsncpy_s(tab->display_name, 32, L"Drives", 31);
            p->in_drive_list = 1;
            /* rebuild a minimal drive list */
            free(p->entries);
            p->entries = (FileEntry *)calloc(64, sizeof(FileEntry));
            p->entry_count = 0;
            p->entries[0].type = ENTRY_DIR;
            wcscpy_s(p->entries[0].name, 256, L"C:\\");
            p->entry_count = 1;
        }
    } else {
        wchar_t drive_letter = towupper(entry_name[0]);
        if (p->drive_paths[drive_letter - L'A'][0] &&
            fs_local.exists(p->drive_paths[drive_letter - L'A']) &&
            fs_local.is_dir(p->drive_paths[drive_letter - L'A'])) {
...
void panel_cursor_home(Panel *p) {
    p->tabs[p->active_tab].cursor = 0;
    p->tabs[p->active_tab].scroll_offset = 0;
    p->dirty = 1;
}
...
void panel_toggle_tag(Panel *p) {
    ...
    p->tagged = (int *)realloc(p->tagged, (p->tagged_count + 1) * sizeof(int));
...
void panel_render(const Panel *p, const Theme *theme, int x, int y, int w, int h, int focused) {
    ...
    } else if (wcsncmp(tab->path, L"\\\\ssh\\", 6) == 0) {
    ...
    for (int i = 0; i < list_h; i++) {
        ...
        if (is_dir) swprintf_s(size_buf, 32, L"<DIR>");
    ...
    if (p->entry_count > list_h) {
        int pct = p->entry_count > 0 ? (tab->cursor * 100) / p->entry_count : 0;
